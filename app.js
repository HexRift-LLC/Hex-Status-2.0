...
class HexStatus {
    ...
    loadConfig() {
        try {
            ...
            console.log(chalk.red('[Config]'), 'Error loading configuration:', error.message);
            process.exit(1);
        }
    }
    ...
    setupConfigWatcher() {
        ...
        this.#configWatcher = fs.watch('./config/config.yml', (eventType, filename) => {
            if (eventType === 'change') {
                try {
                    ...
                    const newServices = newConfig.services.filter(
                        newSvc => !this.config.services.some(
                    ...
                    const removedServices = this.config.services.filter(
                        oldSvc => !newConfig.services.some(
                    ...
                    newServices.forEach(service => {
                        console.log(chalk.green('[Config]'), `New service added: ${service.name}`);
                        io.emit('serviceAdded', service);
                    });
                    
                    // Notify about removed services
                    removedServices.forEach(service => {
                        console.log(chalk.yellow('[Config]'), `Service removed: ${service.name}`);
        ...
        try {
            ...
            if (this.config.discord && this.config.discord.enabled) {
                try {
                    ...
                    console.log(chalk.yellow('[Discord]'), 
                        'Discord integration disabled due to error:', error.message);
                }
            }
            ...
            this.server.listen(this.config.app.port, () => {
                console.log(chalk.green("[Server]"), `Running on port ${this.config.app.port}`);
    ...
    calculateUptime(downtimes) {
        ...
        const totalDowntime = downtimes.reduce((total, downtime) => {
            ...
            if (new Date(downtime.start).getTime() > dayAgo || 
                (downtime.end && new Date(downtime.end).getTime() > dayAgo)) {
                    
                const start = Math.max(dayAgo, new Date(downtime.start).getTime());
                const end = downtime.end ? Math.min(now, new Date(downtime.end).getTime()) : now;
    ...
    async monitorServices() {
        ...
        const monitoringPromises = this.config.services.map(service => this.monitorService(service));
        ...
        if (this.discordBot && typeof this.discordBot.updateStatusEmbed === 'function') {
            try {
                // Convert your serviceData Map to an array of service objects
                const servicesDataArray = Array.from(this.serviceData.entries()).map(([name, data]) => {
                    return {
                        name,
                        status: data.status,
                        ping: data.ping,
    ...
    checkFlapping(serviceName, data, newStatus) {
        ...
        if (data.flappingUntil && data.flappingUntil > now) {
            console.log(chalk.yellow('[Monitor]'), 
                `Service ${serviceName} in flapping cooldown period until ${new Date(data.flappingUntil).toLocaleTimeString()}`);
            ...
            if (lastChanges.length === 0 || (now - lastChanges[lastChanges.length - 1]) > HexStatus.STABLE_DURATION) {
                console.log(chalk.green('[Monitor]'), 
                ...
                if (lastChanges.length === 0 || (now - lastChanges[lastChanges.length - 1]) > HexStatus.FLAPPING_COOLDOWN / 2) {
                    console.log(chalk.green('[Monitor]'), 
    ...
    async monitorService(service) {
        try {
            ...
            const data = this.serviceData.get(service.name);
            if (!data) {
                console.log(chalk.red('[Monitor]'), `Service data not found for ${service.name}`);
                return;
            }
            
            if (data.maintenance) {
                console.log(chalk.blue('[Monitor]'), `Skipping ${service.name} (in maintenance mode)`);
                return;
            }
            ...
            try {
                ...
                if (service.url && isValidUrl(service.url)) {
                    ...
                    const timeout = service.timeout || 
                        (this.config.monitoring && this.config.monitoring.pingTimeout) || 5000;
                    const startTime = Date.now();
                    ...
                    try {
                        const pingTimeout = Math.floor(
                            (service.timeout || 
                                (this.config.monitoring && this.config.monitoring.pingTimeout) || 3000) / 1000
                        );
                        
                        result = await ping.promise.probe(service.url || service.host || service.ip, {
                            timeout: pingTimeout,
                            extra: ['-c', '1'],
                        });
            ...
            if (previousStatus !== 'unknown' && previousStatus !== newStatus) {
                if (this.checkFlapping(service.name, data, newStatus)) {
                    ...
                    if (this.discordBot && typeof this.discordBot.updateServiceHistory === 'function') {
                        const pingValue = result.alive ? parseFloat(result.time) : HexStatus.OFFLINE_RESPONSE_TIME;
            ...
            if (!result.alive) {
                ...
                if (!data.isFlapping && 
                    ...
                    (!data.lastNotification || (Date.now() - data.lastNotification) > 30 * 60 * 1000)) {
            ...
            if (result.alive) {
                ...
                const responseQuality = Math.max(0, Math.min(1, 1 - (result.time / 1000)));
                const recoveryRate = HexStatus.HEALTH_RECOVERY_RATE * (0.5 + (responseQuality * 0.5));
    ...
    sendNotification(service, data, errorDetails) {
        ...
        // Log the incident
        console.log(chalk.red('[Incident]'), 
        ...
        const hasActiveIncident = data.incidents.some(inc => inc.status === 'ongoing');
        ...
        if (this.discordBot && this.config.discord && this.config.discord.enabled) {
            try {
                ...
                if (typeof this.discordBot.sendAlert === 'function') {
                    this.discordBot.sendAlert({
                        service: service.name,
                        status: 'down',
                        time: new Date().toISOString(),
                        failures: data.consecutiveFailures,
                        message: `Service ${service.name} is down! ${errorDetails ? `Error: ${errorDetails}` : ''}`
                    }).catch(error => {
                        this.handleDiscordError(error);
                    });
                }
    ...
    sendRecoveryNotification(service, data, responseTime) {
        ...
        console.log(chalk.green('[Recovery]'), `Service ${service.name} has recovered!`);
    ...
    handleDiscordError(error) {
        ...
        if (
            ...
            console.log(chalk.yellow('[Discord]'), 'Connection issue detected. Attempting to reconnect...');
    ...
    async reconnectDiscord() {
        try {
            console.log(chalk.blue('[Discord]'), 'Attempting to reconnect Discord bot...');
            ...
            console.log(chalk.green('[Discord]'), 'Discord bot reconnected successfully');
            ...
            console.log(chalk.red('[Discord]'), 'Reconnection failed:', error.message);
    ...
    handleMonitoringError(serviceName, error) {
        console.log(chalk.red('[Error]'), `Error monitoring ${serviceName}:`, error.message);
        ...
        const previousStatus = data.status;
        data.status = 'down';
        data.healthScore = Math.max(0, data.healthScore - HexStatus.HEALTH_DECAY_RATE * 2);
        data.consecutiveFailures++;
    ...
    calculateStats() {
        ...
        this.serviceData.forEach(data => {
            ...
            if (data.status === 'up' && data.ping.length) {
                const validPings = data.ping.filter(p => p < HexStatus.OFFLINE_RESPONSE_TIME);
        ...
        return {
            ...
            overallHealth: this.config.services.length ? 
                (totalHealth / this.config.services.length).toFixed(1) : 0
        };
    }
    ...
    setupGracefulShutdown() {
        const shutdown = async (signal) => {
            ...
            console.log(chalk.yellow("[System]"), `Graceful shutdown initiated... (${signal || 'manual'})`);
                ...
                if (this.discordBot) {
                    try {
                        if (typeof this.discordBot.cleanup === 'function') {
                            await this.discordBot.cleanup();
                            console.log(chalk.blue("[System]"), "Discord bot cleanup completed");
                        }
                    } catch (error) {
                        console.error(chalk.red("[Discord]"), "Error during bot cleanup:", error.message);
                    }
                }
