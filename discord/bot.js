/**
 * Hex Status 2.0 - Discord Bot Module
...
function loadConfig() {
    try {
        const configData = yaml.load(fs.readFileSync('./config/config.yml', 'utf8'));
...
function createStatusEmbed(metrics) {
    ...
    const embed = new EmbedBuilder()
        ...
        .setTitle(`${statusEmoji} ${config.app.name} Status`)
    ...
    embed.setFooter({
        text: `${config.app.name} • Refreshes every ${Math.round(config.discord.updateInterval / 1000)}s`,
